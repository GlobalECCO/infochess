...
function BuildingBoard(json) {
  ...
  if (json) {
    ...
    if (!hasKing) {
      throw new InvalidMessageError("Initial armies must have a king!");
    }
  }
}
...
BuildingBoard.prototype.setEwPoints = function(points) {
  if (points > 10) {
    throw "No more than 10 points can be allocated to EW!";
  }
  this.ew_points = points;
};
...
BuildingBoard.prototype.getPieces = function() {
  return this.pieces;
};
...
BuildingBoard.prototype.serialize = function() {
  return JSON.stringify({ pieces: this.pieces, ew_points: this.ew_points }, null, 2);
};

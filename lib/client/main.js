...
require([
  ...
  function onSocketUpdate(updateResponse) {
    ...
    if (updateResponse.result && updateResponse.result.msg === "DEFENSE_RESULT") {
      ...
      var flashMessage = function($flash) {
        ...
        //Jquery UI overlay, to support a model message to the user
        var $overlay = ("<div id='modelMessageOverlay' class='ui-widget-overlay ui-front'></div>");
        $(":root").prepend($overlay);

        $flash.fadeIn(500, function() {
          $(this).one('click', function(event) {
            $(this).hide();
            $("#modelMessageOverlay").remove();
          });
        });
      ...
      var $flash = $("#flashes ."+result.type +"."+viewer + "."+outcome);
  ...
  function beginTutorial() {
    Tutorial.show([
        // Step 1
        {
          el: '#king.piece_selector',
          tip: {
            title: 'Test',
            content: 'Some test tip content',
            tipJoint: 'right',
          },
        },
        // Step 2

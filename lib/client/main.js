...
require([
  ...
  function recalculateArmy() {
    ...
    $('#army_selector #ready').removeClass('selectable')
      .animate({'nothing':null}, 1, function() {
        if (building_board.count('king') > 0 && points == building_board.max_points) {
          $(this).addClass('selectable');
        };
    });
  ...
  function addPiece(container, position, className, margin, isImg) {
    var elemType = 'div';
    if (isImg) {
      elemType = 'img';
    }
    var newPieceOnBoard = document.createElement(elemType);
    var x = position.x;
    var y = position.y;
  ...
  function addPsyopCaptureMarker(position) {
    ...
    var newPieceOnBoard = addPiece(container, position, cssclass, PIECE_MARGIN, true);
    setTimeout(function() {
      $(newPieceOnBoard).attr('src', "images/psyop_capture.gif");
    }, 0);
    return newPieceOnBoard;
  }
  ...
  function addInvisiblePiece(position) {
    ...
    var newPieceOnBoard = addPiece(container, position, cssclass, PIECE_MARGIN, true);
    setTimeout(function() {
      $(newPieceOnBoard).attr('src', "images/invisible_piece.gif");
    }, 0);
    return newPieceOnBoard;
  }
  ...
  function updateIW() {
    ...
    $("#psyop_feint #value").text(g_gameState.board.currentPsyOpAttackCost === 2? points(g_gameState.feintCost): "");
    ...
    $("#ew_feint #value").text(g_gameState.board.currentEWAttackCost === 2? points(g_gameState.feintCost): "");
  ...
  function showPsyopTargets(targets) {
    $('.psyop_target').remove();
    showNotification("If you fail to defend, you will lose a highlighted piece.");
    _.each(targets, function(position) {
  ...
  function clearPawnCaptureSources() {
    $(".pawn_capture_source").remove();
  }

  function refreshPawnCaptureHighlights() {
    var sources = $(".pawn_capture_source");
    var targets = $(".pawn_capture_target");
    sources.removeClass('pawn_capture_source');
    targets.removeClass('pawn_capture_target');
    setTimeout(function() {
      sources.addClass('pawn_capture_source');
      targets.addClass('pawn_capture_target');
    }, 0);
  }
  ...
  function updatePawnCaptures(captures) {
    ...
    for (i = 0; i < captures.length; i++) {
      ...
      if (targets.indexOf(capture.dest) === -1) {
        targets.push(capture.dest);
      }
      sources[new Position(capture.src.x, capture.src.y).asKey()] = capture.src;
    }

    showNotification("The following targets can be captured.  Choose one.");

    for (i = 0; i < targets.length; i++) {
      var destinationHandler = function(dest) {
        return function() {
          var dir_mod = getPlayerColour() === "white" ? -1 : 1;
          var left  = new Position(dest.x - 1, dest.y + (1*dir_mod));
          var right = new Position(dest.x + 1, dest.y + (1*dir_mod));
          clearPawnCaptureSources();
          showNotification("Choose the pawn that will make the capture.");

          var addSource = function(position, triggerNow) {
            var handler = function() {
              var move = {
                src: position,
                dest: dest
            ...
            if (triggerNow) {
              ...
              addPawnCaptureSource(position, handler);
            }
          };

          var isLeft = sources[left.asKey()];
          var isRight = sources[right.asKey()];

          if (isLeft && isRight) {
            addSource(left);
            addSource(right);
            refreshPawnCaptureHighlights();
          } else if (isLeft) {
            addSource(left, true);
          } else {
            addSource(right, true);
          }
        };
      }(targets[i]);
      addPawnCaptureTarget(new Position(targets[i].x, targets[i].y), destinationHandler);
    }
  }
  ...
  function updateTurnCount(turn_count) {
    ...
    var turn = g_gameState.getTurnLabel();
    ...
    */
    ...
    $('#army_selector #ready').bind('click', function() {
      if (!g_ready) {
        ...
        var building_board = getBuildingBoard();
        if (building_board.count('king') == 0 || building_board.points() !== building_board.max_points) {
          return;
        }
  ...
  function onSocketUpdate(updateResponse) {
    ...
    $('.invisible_piece').remove();
    $('.psyop_target').remove();
    $('.psyop_capture').remove();
    ...
    if (g_gameState.idleTurnCount === 10) {
      showSuggestOfferDrawDialog();
    }
  ...
  function showSuggestOfferDrawDialog() {
    $("#settings_dialog").css("z-index", "20000");
    $("#settings_dialog").css("visibility", "visible");
    $("#settings_content").css("visibility", "hidden");
    $("#suggest_offer_draw_content").css("visibility", "visible");
  }
  function hideSuggestOfferDrawDialog() {
    $("#suggest_offer_draw_content").css("visibility", "hidden");
    hideSettings();
  }
  ...
  $("#suggest_offer_draw_content .close").bind('click', function() {
    hideSuggestOfferDrawDialog();
  });
  $("#confirm_offer_draw, #confirm_suggest_draw").bind('click', function() {
    offer_draw();
    hideOfferDrawDialog();
    hideSuggestOfferDrawDialog();
    hideSettings();

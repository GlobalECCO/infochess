...
require([
  ...
  function recalculateArmy() {
    ...
    //for every piece type, see if there are any valid positions available
    TYPES.forEach(function(type) {
      var positions = building_board.getPossiblePlacements(Piece(type, getPlayerColour()));
      if (positions.length === 0) {
        //if no positions available un-select this piece type
        $("#piece_selectors #" + type).removeClass("selectable");
      }
      else {
        // Otherwise make sure it has the selectable class
        $("#piece_selectors #" + type).addClass("selectable");
      }
    });

    //"enable" the ready button if the player met all the requirements
    $('#army_selector #ready').toggleClass('selectable', building_board.count('king'));
    ...
    if (last_turn_report.iw_attack.type === 'psyop') {
      if (last_turn_report.iw_attack.success) {
        if (last_turn_report.iw_attack.outcome) {
          ...
          iw_report += Mustache.render("Psyop attack was successful, capturing a {{captured_piece}} at {{position}}.", options);
        } else {
          iw_report += "Psyop attack was successful.";
        }
      } else if (last_turn_report.iw_attack.success === false) { // Could be undefined if attack type was null
        iw_report += "Psyop attack was unsuccessful.";
      }
    } else if (last_turn_report.iw_attack.type === 'ew') {
      if (last_turn_report.iw_attack.success) {
        iw_report += "E-warfare attack was successful.";
      } else if (last_turn_report.iw_attack.success === false) { // Could be undefined if attack type was null
        iw_report += "E-warfare attack was unsuccessful.";
      }
    }
  ...
  function initPawnUpgradeDialog() {
    ...
    for (var i = 0; i < pieces.length; i++) {
      ...
      $(div).click(function() {
          ...
          $dialog = $("#pawn_upgrade_dialog").css('z-index', '0').css('visibility', 'hidden');
        });
      container.appendChild(div);
    }
  }
  ...
  function onSocketError(msg) {
    printMessage("server", "Error: " + msg.message + " - please file a bug report.", "server");
  ...
  function onSocketUpdate(updateResponse) {
    ...
    if (updateResponse.result && updateResponse.result.msg === "DEFENSE_RESULT") {
      ...
      var flashMessage = function($flash) {
  ...
  Tutorial.init(socket, onSocketUpdate, function() {
    g_ready = false;
    removeTempPieces();
    removeSolidPieces();
    removeShadowPieces();

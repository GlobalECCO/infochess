...
var metadata = {
  ...
  roles: [
    ...
    { name: "Observer", slug: ROLES.OBSERVER, optional: true },
  ]
};
...
InfoChess.prototype.asDTO = function(role) {
  var dto = {
    ...
    idleTurnCount: this.idleTurnCount,
  };

  // During setup phase, we may have a pending submitted army.
  if (this.currentPhase === this.PHASES.SETUP) {
    if (!role || role === this.ROLES.OBSERVER) {
      if (this.initialArmies[this.ROLES.WHITE]) {
        dto.builtArmyRole = this.ROLES.WHITE;
      } else if (this.initialArmies[this.ROLES.BLACK]) {
        dto.builtArmyRole = this.ROLES.BLACK;
      }
    } else if (this.initialArmies[role]) {
      dto.builtArmyRole = role;
    }

    if (dto.builtArmyRole) {
      dto.builtArmy = this.initialArmies[dto.builtArmyRole].serialize();
    }
  }
  ...
  if (this.board) {
    ...
    if (role) {
      ...
      dto.board = this.board.getViewPieces(viewRole);
      ...
      if (role !== ROLES.OBSERVER && this.last_turn_report && this.last_turn_report.move.invisible && role !== this.last_turn_report.role) {
...
InfoChess.prototype.fromDTO = function(data) {
  ...
  if (data.builtArmy && data.builtArmyRole) {
    this.builtArmy = new BuildingBoard(data.builtArmy);
    this.initialArmies[data.builtArmyRole] = this.builtArmy;
  }
  ...
  this.idleTurnCount = data.idleTurnCount;
};
...
InfoChess.prototype.startGame = function() {
  ...
  this.notifyNewTurn(this.ROLES.WHITE);
};
...
InfoChess.prototype.endTurn = function(role) {
  ...
  // Do some checking to see if this turn was idle
  if (!this.currentTurn.move.captured_piece) {
    this.idleTurnCount++;
  } else {
...
InfoChess.prototype.notifyNewTurn = function() {
  this.addHistory(this.HISTORY_TYPES.TURN_STARTED, "Turn #" + this.getTurnLabel() + ".");
};

InfoChess.prototype.getTurnLabel = function() {
  if (!this.turn_count || this.turn_count === 0) {
    return 'None';
  }

  var turn = Math.floor(((this.turn_count - 1) / 2) + 1);
  if ((this.turn_count - 1) % 2) {
    turn += ' Black';
  } else {
    turn += ' White';
  }
...
InfoChess.prototype.pawn_upgrade = function(role, new_type) {
  ...
  this.addHistory(this.HISTORY_TYPES.PAWN_UPGRADE, role + " upgraded their pawn to a " + new_type + ".");
...
InfoChess.prototype.iw_attack = function(role, options) {
  ...
  if (type === 'psyop') {
    this.addHistory(this.HISTORY_TYPES.PSYOP_ATTACK, {
      ...
      actor: role + " issued a " + options.strength + " psyop attack!",
      opponent: role + " issued a psyop attack!"
    }, role);
  } else {
    this.addHistory(this.HISTORY_TYPES.PSYOP_ATTACK, {
      game: role + " issued a " + options.strength + " electronic warfare attack!",
      actor: role + " issued a " + options.strength + " electronic warfare attack!",
...
InfoChess.prototype.iw_defense = function(role, options) {
  ...
  if (options.defend === true) {
    ...
    if (this.current_iw_attack.strength !== 'reinforced') {
      ...
      // EW/Psyop normal/feint, defense successful
  ...
  if (this.current_iw_attack.type === 'psyop') {
    ...
    if (this.current_iw_attack.strength !== 'feint') {
      ...
      if (options.defend === true) {
        // Psyop reinforced, defended
        this.addHistory(this.HISTORY_TYPES.IW_DEFEND, role + " defended but failed and lost a " + result.captured_piece.type + " at " + result.captured_position.humanise() + " anyway!");
      } else {
        // Psyop normal/reinforced, ignored
        this.addHistory(this.HISTORY_TYPES.IW_DEFEND, role + " ignored the attack and lost a " + result.captured_piece.type + " at " + result.captured_position.humanise() + "!");
      }
    } else {
      // Psyop feint, ignored
      this.currentTurn.iw_attack.success = false;
      this.addHistory(this.HISTORY_TYPES.IW_DEFEND, role + " ignored the attack, but it was a feint so there was no effect.");
    }
  } else {
    this.board.cycleEWCosts();
    if (this.current_iw_attack.strength !== 'feint') {
      this.currentTurn.iw_attack.success = true;
      if (options.defend === true) {
        // EW reinforced, defended
        this.addHistory(this.HISTORY_TYPES.IW_DEFEND, role + " defended but failed and lost their next move anyway.");
      } else {
        // EW normal/reinforced, ignored
        this.addHistory(this.HISTORY_TYPES.IW_DEFEND, role + " ignored the attack and lost their next move.");
      }
    } else {
      // EW feint, ignored
      this.currentTurn.iw_attack.success = false;
...
InfoChess.prototype.move = function(role, src, dest) {
  ...
  var was_invisible = pieces[src_pos.asKey()].invisible === true;
  var result = this.board.move(role, src, dest, this.currentPhase);

  var default_opponent_msg = role + " made a hidden move.";
  this.currentTurn.move.moving_piece = result.moving_piece.type;

  if (was_invisible) {
    // Source is not revealed if the attacking piece is invisible
    delete result.src;
  } else {
  ...
  if (result.type === 'pawnbump') {
    ...
    this.addHistory(this.HISTORY_TYPES.MOVE, {
      ...
      actor: role + " castled your king from " + src.humanise() + " to the rook at " + dest.humanise() + ".",
      opponent: default_opponent_msg
    }, role);
  } else if (result.type === 'capture') {
    this.currentTurn.move.captured_piece = result.captured_piece.type;
    if (was_invisible) {
      this.addHistory(this.HISTORY_TYPES.MOVE, role+"'s "+result.moving_piece.type+" moved to "+dest.humanise()+", capturing a "+result.captured_piece.type + "!");
    } else {
    ...
    if (result.moving_piece.invisible) {
      ...
      this.addHistory(this.HISTORY_TYPES.MOVE, {
        game: role + "'s " + result.moving_piece.type + " moved from " + src.humanise() + " to " + dest.humanise() + ".",
        actor: role + "'s " + result.moving_piece.type + " moved from " + src.humanise() + " to " + dest.humanise() + ".",
        opponent: default_opponent_msg
      }, role);
    } else {
      this.addHistory(this.HISTORY_TYPES.MOVE, role + "'s " + result.moving_piece.type + " moved from " + src.humanise() + " to " + dest.humanise() + ".");
    }
  }
...
InfoChess.prototype.getHistory = function(role) {
  this.validateRole(role);
  if (this.isGameOver() || role === this.ROLES.OBSERVER) {
...
InfoChess.prototype.addHistory = function(type, messageOrObj, actor_role) {
  ...
  var handleObject = function(type, options) {
  ...
  if (typeof messageOrObj == 'string') {
    ...
    handleObject(type, messageOrObj);
  } else {

...
PlayingBoard.prototype.getPawnCaptures = function(role) {
  ...
  if (this.last_move && this.last_move.moving_piece) {
    ...
    var src = {x: this.last_move.dest.y, y: this.last_move.starting_row};
...
PlayingBoard.prototype.isLastMoveEnPassant = function() {
  return this.last_move &&
    ...
    Math.abs(this.last_move.dest.y - this.last_move.moving_piece.starting_row) === 2;
};
...
PlayingBoard.prototype.getPossibleMoves = function(piece, position) {
  ...
  var addIfOccupied = function(position, includeEnPassant) {
    var target = pieces[position.asKey()];
    if (target && target.colour !== piece.colour) {
      return add(position);
    }
    if (includeEnPassant && me.isLastMoveEnPassant()) {
      var dest = new Position(me.last_move.dest.x, me.last_move.dest.y + direction);
      if (position.asKey() == dest.asKey()) {
        add(position);
      }
    }
    return false;
  ...
  if (piece.type === 'pawn') {
    ...
    //Attack vectors!
    addIfOccupied(new Position(position.x+1, position.y+(1*direction)), true);
    addIfOccupied(new Position(position.x-1, position.y+(1*direction)), true);

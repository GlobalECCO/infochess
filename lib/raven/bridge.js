    ...
    function addPlayer(socket, user, role) {
      ...
      var logAndHandleAndUpdate = function(message, callback) {
        socket.on(message, function(data) {
          ...
          if (role === OBSERVER_ROLE) {
            NotifyError({message:"Observer attempted to save the game state"});
            return;
          }
      ...
      logAndHandleAndUpdate('select_army', function(serializedArmy) {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to select their army");
        }
      ...
      logAndHandleAndUpdate('unready', function() {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to unready their army");
        }
      ...
      logAndHandle('move', function(move) {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to move a piece");
        }
      ...
      var handleIW = function(type) {
        if (type !== 'ew' && type !== 'psyop') {
          NotifyError({message:"Type must be 'ew' or 'psyop'"});
          return;
        }

        return function(data) {
          if (role === OBSERVER_ROLE) {
            throw new InvalidMessageError("Observer attempted to perform an IW attack");
          }

          if (typeof data.strength === 'undefined') {
            throw new InvalidMessageError("'strength' must be specified for IW attacks");
          }
      ...
      logAndHandle('iw_defense', function(data) {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to perform an IW defense");
        }
      ...
      logAndHandleAndUpdate('pawn_upgrade', function(data) {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to upgrade a pawn");
        }
      ...
      logAndHandle('pawn_capture_query', function() {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to perform a pawn capture");
        }
      ...
      logAndHandleAndUpdate('end_turn', function(data) {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to end the turn");
        }
      ...
      socket.on('forfeit', function(data) {
        if (role === OBSERVER_ROLE) {
          throw new InvalidMessageError("Observer attempted to forfeit the game");
        }
